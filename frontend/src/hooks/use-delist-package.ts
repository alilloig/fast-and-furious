...
export function useDelistPackage() {
  ...
  return useMutation({
    mutationFn: async ({ listingId, sellerCapId }: DelistPackageArgs) => {
      const tx = new Transaction();
      tx.moveCall({
        target: `${MARKETPLACE_PACKAGE_ID}::package_listing::delist`,
        arguments: [
          tx.object(listingId),
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["package-listings"] });
    },
  });
}
